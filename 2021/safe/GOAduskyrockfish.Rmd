---
title: "12. Assessment of the Dusky Rockfish stock in the Gulf of Alaska"
output: 
...
author: "Benjamin C. Williams, Peter-John F. Hulson, Chris R. Lunsford, Kari H. Fenske"
date: "November 2021"
...
df = readLines(here::here("2021", "proj", "author_f", "bigsum.dat"), skip = 2)
df1 <- as.data.frame(do.call(rbind, strsplit(df[-1], " ")))
names(df1) <- strsplit(df[1], ' ')[[1]]

df1 %>% 
  dplyr::filter(Alt == 1, Year == year + 1) %>% 
   mutate(ABC = round(as.numeric(ABC) * 1000),
          OFL = round(as.numeric(OFL) * 1000)) -> df
df %>% pull(OFL) -> ofl
df %>% pull(ABC) -> maxABC
```
