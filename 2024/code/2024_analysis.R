...
# Why are the nnumbers changing from this year compared to 2022?
...
afscassess::plot_params_dusk(year = year, folder = paste0(alt_mod,"/mcmc"), model_name = "base",
...
egoa_frac <- read.csv(here::here(year, alt_mod, "apport", "egoa_prop_vast.csv")) %>% mutate(value = as.character(wyak))
...
mcmc_eval_summary_duskfunction(year, model)
...
## also random things to read in later:
#
# check out model outputs, convergence, looks ok
# then run mcmc for basic model (non-final models) --> 100,000 save 200 (total ~ 1000 saved or so)
# final model for mcmc run -mcmc 10,000,000 -mcsave 2000 (~6 hrs) Ben has a virtual machine --> ask IT for access to a virtual machine
# next run process_results() this will make the folders etc...
#     processed (mcmc processed results), projections, figs, tables, retro
...
#update end year
#dat file name
#yield ratio (comes from clean_catch)

# now we can directly pull the TAC values: AKR.V_CAS_TAC

# note: must provide file for VAST or DB estimates are output

# random pieces to look into
